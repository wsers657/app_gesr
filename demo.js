(() => {
  ...
  const initialTransactions = $('#workspaceTransactions').innerHTML;
  ...
  function allocateNetflixSavings(){if(savingsAllocated||!items.find(item=>item.id==='netflix')?.optimized)return;const netflixSaving=model.monthlySaving(model.reset().find(item=>item.id==='netflix'));savingsAllocated=true;goalSaved+=netflixSaving;householdSavings+=netflixSaving;renderGoal();$('#allocateSavings').disabled=true;$('#allocateSavings').textContent='✓ 7 € / mois affectés';$('#savingAllocation').classList.add('allocated');$('#goalFeedback').textContent='✓ Premier versement ajouté. Les 7 € économisés seront transférés chaque mois.';setTimeout(()=>{showWorkspace();selectWorkspaceTab('goals');},550);}
  ...
  window.addEventListener('hashchange',()=>{if(location.hash.split('/')[0]==='#demo')reset();else {analysisTimers.forEach(clearTimeout);$('#demoDrawer').hidden=true;document.body.classList.remove('drawer-open');}});
